"""
Provides functions to compute several measures of th Implied Cost of Capital.
"""
...
class icc(wrds_module):
    ...
    def hdz_ct(self, data, freq='Q', ind_vars=None):
        ...
        df['M'] = self.w.comp.get_fields(['prccq'], df)
        # Get the growth rate g
        df['g'] = self.w.fred.get_10y_US_rates(df, col_date='datadate')
        df['g'] = df.g - 0.03
        ...
        df_split = np.array_split(df[:1000], self.w.cores)
        pool = mp.Pool(self.w.cores)
    ...
    def hdz_gls(self, data, freq='Q', ind_vars=None):
        ...
        # Compute the cost of capital
        # Apply the function in parallel
        df_split = np.array_split(df, self.w.cores)
        pool = mp.Pool(self.w.cores)
    ...
    def hdz_oj(self, data, freq='Q', ind_vars=None):
        ...
        df['M'] = self.w.comp.get_fields(['prccq'], df)
        # Get the growth rate g
        df['gamma'] = self.w.fred.get_10y_US_rates(df, col_date='datadate')
    ...
    def _earnings_forecasts_hdz(self, data, freq='Q', N=5, ind_vars=None):
        ...
        Argments:
            ...
            N --    Defnines how many future forecasts are needed
        ...
        dc = self.w.comp.get_fields(fields).drop_duplicates()
        dc.columns = key+fields_names
        # Add additional independent variables from the user data
        dfu = self.w.open_data(data, key + ind_vars).drop_duplicates()
