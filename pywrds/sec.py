...
class sec13f(wrds_module):
    ...
    def get_io_perc(self, data):
        ...
        su = self.w.open_data(self.summary, cols).drop_duplicates()
        ...
        hol = self.w.open_data(self.holdings, cols)
        ...
        m['permno'] = self.w.crsp.permno_from_cusip(m)
        ...
        # link = self.w.open_data(self.link, ['cik', 'mgrno',
        #                                     'matchrate', 'flag'])
        # cr = cr.merge(link, how='left', on='cik')
        # # # Flag the mgrno that are present on TR-S34 after 2013-06-30
        # # s34 = self.w.open_data(self.w.tr13f.type1, ['mgrno', 'fdate'])
