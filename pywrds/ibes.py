...
class ibes(wrds_module):
    ...
    def _ibes_crsp_linktable(self):
        ...
        _ibes1 = self.w.open_data(self.id, cols)
        ...
        _crsp1 = self.w.open_data(self.w.crsp.stocknames, cols)
        ...
        ibesid = self.w.open_data(self.id, cols)
        ...
        _crsp_n1 = self.w.open_data(self.w.crsp.stocknames, cols)
    ...
    def _adjustmentfactors_table(self):
        ...
        df_adj = self.w.open_data(self.adj, cols)
    ...
    def _get_adjustment_factors(self, data):
        ...
        """
        key = ['ticker', 'fpedats']
        dfu = self.w.open_data(data, key).drop_duplicates()
    ...
    def ibesticker_from_gvkey(self, data):
        ...
        # Add the permno
        key = ['gvkey', 'datadate']
        df = self.w.open_data(data, key).drop_duplicates().dropna()
        df['permno'] = self.w.crsp.permno_from_gvkey(df)
        # Add the IBES ticker
    ...
    def get_fpedats(self, data):
        ...
        """
        df = self.w.open_data(self.det, ['ticker',
                                         'fpedats']).drop_duplicates()
        dt = pd.to_datetime(df.fpedats).dt
        df['yr'] = dt.year
        df['mon'] = dt.month
        dfu = self.w.open_data(data, ['ticker', self.col_fpedats])
    ...
    def _get_fields_det(self, fields=None):
        ...
        key = ['ticker', 'fpedats']
        # Filter measure
        det = self.w.open_data(self.det, ['measure'])
        kmeasure = det.measure == self.measure
        # Filter usfirm
        det = self.w.open_data(self.det, ['usfirm'])
        kusfirm = det.usfirm == self.usfirm
        # Filter fpi
        det = self.w.open_data(self.det, ['fpi'])
        kfpi = det.fpi.isin(self.fpis)
        # Filter pdf (Primary / Diluted)
        if self.pdf == 'All':
            kpdf = True
        elif self.pdf == 'P' or self.pdf == 'D':
            det = self.w.open_data(self.det, ['pdf'])
            kpdf = det.pdf == self.pdf
        elif self.pdf == 'DP':
            det = self.w.open_data(self.det, key+['pdf'])
            pdfd = det.pdf == 'D'
            pdfp = det.pdf == 'P'
        ...
        det = self.w.open_data(self.det, fields)
    ...
    def _get_fields_guidance(self, fields=None):
        ...
        df = self.w.open_data(self.guidance, cols+fields)
    ...
    def get_management_forecasts(self, data, fields, which='last',
        ...
        if self.col_fpedats in data_cols:
            ...
            dfu = self.w.open_data(data, ['ticker', 'fpedats'])
    ...
    def _get_ea_fields(self, fields):
        ...
        # ea = self.w.open_data(self.det, cols)
    ...
    def get_consensus(self, data, startdt, enddt, for_quarter=0, n=5,
        ...
        cdt = self.w.open_data(data, key + [startdt, enddt]).drop_duplicates()
    ...
    def get_ptg(self, data, startdt, enddt, measure='PTG',
        ...
        det = self.w.open_data(self.ptg, ['measure'])
        kmeasure = det.measure == measure
        # Filter usfirm
        det = self.w.open_data(self.ptg, ['usfirm'])
        kusfirm = det.usfirm == self.usfirm
        # Filter horizon
        det = self.w.open_data(self.ptg, ['horizon'])
        khorizon = det.horizon == horizon
        # Create the final filter
        mask = kmeasure & kusfirm & khorizon
        cols = ['ticker', 'value', 'anndats', 'anntims', 'amaskcd']
        det = self.w.open_data(self.ptg, cols)
        det = det[mask].drop_duplicates()
